...
export const NavBody = ({ children, className, visible }: NavBodyProps) => {
  return (
    <div
      ...
      className={cn(
        "relative z-[60] mx-auto hidden flex-row items-center justify-between self-start rounded-full bg-transparent px-4 py-2 transition-all duration-300 xl:flex dark:bg-transparent",
        visible
          ? "w-[88%] max-w-[1120px] translate-y-5 bg-white/80 dark:bg-neutral-950/80"
...
export const NavItems = ({ items, className, onItemClick }: NavItemsProps) => {
  ...
  return (
    <div
      ...
      className={cn(
        "absolute left-1/2 top-1/2 z-10 hidden -translate-x-1/2 -translate-y-1/2 flex-row items-center justify-center gap-1 text-sm font-medium text-zinc-600 transition duration-200 hover:text-zinc-800 xl:flex",
        className,
      )}
    >
      {items.map((item, idx) => (
        ...
        >
          ...
          >
            {hovered === idx && (
              <div
                className="absolute inset-0 h-full w-full rounded-full bg-gray-100 shadow-[0_8px_20px_rgba(15,23,42,0.08)] dark:bg-neutral-800"
              />
            )}
          ...
          {item.dropdown && item.dropdown.length > 0 && hovered === idx && (
            <div className="absolute left-1/2 top-[calc(100%+0.75rem)] z-50 w-60 -translate-x-1/2 animate-in fade-in-0 slide-in-from-top-2 duration-150">
              <div className="overflow-hidden rounded-3xl border border-neutral-200/80 bg-white/95 p-2 shadow-[0_24px_60px_rgba(15,23,42,0.12)] backdrop-blur-xl">
                {item.dropdown.map((subItem) => (
                  <Link
                    key={subItem.link}
                    href={subItem.link}
                    onClick={onItemClick}
                    className="flex items-center justify-between rounded-2xl px-3 py-2.5 text-sm font-medium text-neutral-700 transition-colors hover:bg-neutral-100 hover:text-neutral-950"
                  >
...
export const MobileNavMenu = ({
  ...
  return (
    isOpen ? (
      <div
        className={cn(
          "absolute inset-x-0 top-16 z-50 flex w-full flex-col items-start justify-start gap-4 rounded-lg bg-white px-4 py-8 shadow-[0_0_24px_rgba(34,_42,_53,_0.06),_0_1px_1px_rgba(0,_0,_0,_0.05),_0_0_0_1px_rgba(34,_42,_53,_0.04),_0_0_4px_rgba(34,_42,_53,_0.08),_0_16px_68px_rgba(47,_48,_55,_0.05),_0_1px_0_rgba(255,_255,_255,_0.1)_inset] animate-in fade-in-0 duration-150 dark:bg-neutral-950",
          className,
        )}
      >
        {children}
